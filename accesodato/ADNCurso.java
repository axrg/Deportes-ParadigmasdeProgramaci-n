...
public class ADNCurso {
    ...
    {
        int r=0;
        String sql="insert into deportes values(null,?,?)";
    ...
    {
        int r=0;
        String sql="update deportes set nombre=? where idcurso=?";
        try (Connection cn= Conexion.Cadena();
                PreparedStatement ps= cn.prepareStatement(sql)){
            ps.setString(1,p.getNombre());
    ...
    {
        int r=0;
        String sql="select ifnull(count( idcurso),0) from deportes where idcurso=?";
        try (Connection cn= Conexion.Cadena();
                PreparedStatement ps= cn.prepareStatement(sql)){
    ...
    {
        int r=0;
        String sql="delete from deportes where idcurso=?";
        try (Connection cn= Conexion.Cadena();
                PreparedStatement ps= cn.prepareStatement(sql)){
            ps.setInt(1,p.getIdcurso());
            r=ps.executeUpdate();
    ...
    private static LinkedList<Curso> Combo() throws ClassNotFoundException, SQLException
    {
        int r=0;
        LinkedList<Curso> Lista = new LinkedList<>();
        String sql="select idcurso,nombre from deportes";
        try (Connection cn= Conexion.Cadena();
                PreparedStatement ps= cn.prepareStatement(sql)){
            try (ResultSet rs= ps.executeQuery()){
               while(rs.next())
    ...
    {
        int r=0;
        String sql="select idcurso from deportes where nombre=?";
        try (Connection cn= Conexion.Cadena();
                PreparedStatement ps= cn.prepareStatement(sql)){
            ps.setString(1,p.getNombre());
            try (ResultSet rs= ps.executeQuery()){
